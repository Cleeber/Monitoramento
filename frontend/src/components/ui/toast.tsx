...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-lg border p-4 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full backdrop-blur-sm",
  {
    variants: {
      variant: {
        default: "border-gray-700 bg-gray-800/90 text-gray-100 shadow-xl",
        destructive:
          "border-red-500/30 bg-red-900/20 text-red-100 shadow-xl backdrop-blur-sm",
        success:
          "border-green-500/30 bg-green-900/20 text-green-100 shadow-xl backdrop-blur-sm",
      },
    },
    defaultVariants: {
      variant: "default",
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      "absolute right-2 top-2 rounded-md p-1 text-gray-400 opacity-70 transition-all hover:text-gray-200 hover:opacity-100 focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-gray-500 group-hover:opacity-100 group-[.destructive]:text-red-300 group-[.destructive]:hover:text-red-100 group-[.destructive]:focus:ring-red-500 group-[.success]:text-green-300 group-[.success]:hover:text-green-100 group-[.success]:focus:ring-green-500",
      className
    )}
    toast-close=""
    {...props}
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Title
    ref={ref}
    className={cn("text-sm font-semibold text-gray-100 leading-tight", className)}
    {...props}
  />
))
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Description
    ref={ref}
    className={cn("text-sm text-gray-300 opacity-95 leading-relaxed", className)}
    {...props}
  />
))
